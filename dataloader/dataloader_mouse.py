...
from data_utils import train_val_test_split, normalize, concatenate_behaviors
...
def dataloader_mouse_wheel(fpath_lst, **data_params):
    ...
    for i, fpath in enumerate(fpath_lst):
        ...
        sess_name = f'Animal_{subject_id}-Session_{session_date}-Area_{area}'

        train_dataset[sess_name] = {'neural_data': [], 'behavior_data': [], 'trials_type': [], 'trials_length': []}
        valid_dataset[sess_name]   = {'neural_data': [], 'behavior_data': [], 'trials_type': [], 'trials_length': []}
        test_dataset[sess_name]  = {'neural_data': [], 'behavior_data': [], 'trials_type': [], 'trials_length': []}
        dataset_info[sess_name]  = {'train_trials': None, 'valid_trials': None, 'test_trials': None, 'obs_dim': None}
        ### Get the train-valid-test split based on trials ###
        num_trials = results['num_trials']
        train_trials, valid_trials, test_trials = train_val_test_split(num_trials, data_params['val_size'], data_params['test_size'], seed)
        ...
        dataset_info[sess_name]['val_trials']   = valid_trials 
        ...
        valid_behv_data   = {key: [value[i] for i in valid_trials] for key, value in behv_data.items()}
        ...
        valid_trials_len   = [trial_lengths[k] for k in valid_trials]
        ...
        valid_trials_type = [trial_types[k] for k in valid_trials]
        ...
        for j_behv, key in enumerate(data_params['behavior_keys']):
            ...
            num_valid = len(valid_behv_data[key])
            train_key_data = train_behv_data[key]
            valid_key_data = valid_behv_data[key]
            test_key_data  = test_behv_data[key]
            if key not in ['lick', 'speed_class']:
                train_key_data,  valid_key_data,  test_key_data = normalize(data_params['behavior_normalizor'], train_key_data, valid_key_data, test_key_data)
            behv_data_train_norm_dic[key] = train_key_data
            if len(valid_trials) != 0:
                behv_data_valid_norm_dic[key] = valid_key_data
        ...
        valid_behv_data   = concatenate_behaviors(behv_data_valid_norm_dic, num_valid, data_params['behavior_keys']) if num_valid_trials != 0 else None
        test_behv_data  = concatenate_behaviors(behv_data_test_norm_dic, num_test, data_params['behavior_keys']) if num_test_trials != 0 else None
        ######################
        ...
        valid_dataset[sess_name]['trials_type'] = valid_trials_type
